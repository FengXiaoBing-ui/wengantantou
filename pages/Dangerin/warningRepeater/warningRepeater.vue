<template>
	<view class="index">
		<headerTab title="中继器告警" @option="option" @func="func" :record="record" :confirmed="confirmed" :garbage="garbage"></headerTab>
		<view class="wrap" :class="wraptop == true ? 'longtap' : ''">
			...
			>
				...
				<view class="list-left">
					...
					<text class="mid">中继器编号{{ item.device_id }}</text>
					<view class="bot">
...
export default {
	...
	methods: {
		...
		alllistsure() {
			...
			if(!this.record){
				this.$api.postapi('/api/Firealarm/del_alarms',{ids:ids,index:1}).then(res => {
					console.log(res)
					if(res.data.code==0){
						uni.showToast({
							title:"该数据还未处理，不能删除",
			...
			this.$api.postapi('/api/Alarmlog/confirm_choose_warnning', { ids: ids,index: 1, login_id: uni.getStorageSync('loginId') }).then(res => {
				console.log(res);
			});
			for (let i = 0; i < this.warninglist.length; i++) {
				this.warninglist[i].active = '';
